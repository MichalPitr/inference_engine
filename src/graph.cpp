...
void Graph::replaceNode(Node* oldNode, std::unique_ptr<Node> newNode) {
    // Note: This assumes that the new node has the same connections as the old
    // one. If this is not the case, parents/children need to updated manually.
...
bool Graph::isInputNode(Node* node) const {
    return std::any_of(node->getInputs().begin(), node->getInputs().end(),
                       [this](const std::string& input) {
                           return std::find(inputs_.begin(), inputs_.end(),
                                            input) != inputs_.end();
                       });
}

void Graph::topologicalSortUtil(Node* node, std::unordered_set<Node*>& visited,
                                std::stack<Node*>& stack) {
    visited.insert(node);
