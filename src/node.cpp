...
OpType Node::getOpType() const { return opType; }

const std::vector<std::string> &Node::getInputs() const { return inputs; }

const std::vector<std::string> &Node::getOutputs() const { return outputs; }

void Node::addInput(std::string input) { inputs.push_back(input); }

void Node::addOutput(std::string output) { outputs.push_back(output); }
...
template std::optional<std::vector<int64_t>> Node::getAttribute(
